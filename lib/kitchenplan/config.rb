...
module Kitchenplan
  ...
  class Config
    ...
    def parse_people_config
        ...
        @people_config = ( YAML.load(ERB.new(File.read(people_config_path)).result) if File.exist?(people_config_path) ) || YAML.load(ERB.new(File.read("config/people/brightcove.yml")).result)
    end
